...
func TestRunIKSExample(t *testing.T) {
	...
	options.PreDestroyHook = func(o *testhelper.TestOptions) error {
		// -refresh=false prevents the provider's Read from firing before destroy.
		// Without this, a stale connection ID in state causes the IBM provider to
		// hard-error (HTTP 400 "does not exist") during the pre-destroy refresh,
		// before Delete is even attempted. With -refresh=false the Delete is called
		// directly and the connection is properly removed from BRS.
		// Remove once https://github.com/IBM-Cloud/terraform-provider-ibm/pull/6906
		// is merged and released (provider will self-heal on Read instead).
...
func TestRunOCPExample(t *testing.T) {
	...
	options.PreDestroyHook = func(o *testhelper.TestOptions) error {
		// -refresh=false prevents the provider's Read from firing before destroy.
		// Same reason as IKS above — avoids hard-error on stale connection IDs,
		// while still allowing Delete to be called and remove the connection from BRS.
		o.TerraformOptions.ExtraArgs.Destroy = append(o.TerraformOptions.ExtraArgs.Destroy, "-refresh=false")
		return nil
	}
...
func TestRunCrossClusterExample(t *testing.T) {
	...
	options.PreDestroyHook = func(o *testhelper.TestOptions) error {
		// -refresh=false prevents the provider's Read from firing before destroy,
		// avoiding a hard-error on stale connection IDs (same reason as IKS/OCP).
		// Delete is still called for both connections so BRS is cleaned up properly.
		// parallelism=1 ensures cluster worker-node cleanup finishes before
...
func TestRunCrossClusterExistingConnection(t *testing.T) {
	...
	options.PreDestroyHook = func(o *testhelper.TestOptions) error {
		...
		// avoiding a hard-error on stale connection IDs (same reason as IKS/OCP).
		// Delete is still called for both connections so BRS is cleaned up properly.
		// parallelism=1 ensures cluster worker nodes finish draining before
		// Terraform attempts to delete the VPC and its subnets (avoids "VPC is in use").
		o.TerraformOptions.ExtraArgs.Destroy = append(o.TerraformOptions.ExtraArgs.Destroy, "-refresh=false", "-parallelism=1")
		return nil
	}
