...
output "recovery_type" {
  ...
  value       = local.is_full_recovery ? var.recovery_type : null
}

output "recovery_protection_group_name" {
  description = "Name of the protection group used for recovery"
  value       = local.is_full_recovery ? local.recovery_pg_name : null
}
...
output "recovery_namespace_prefix" {
  ...
  value       = local.is_full_recovery ? var.recovery_namespace_prefix : null
}
...
output "target_cluster_connection_id" {
  ...
  value       = local.is_full_recovery && var.recovery_type == "cross-cluster" ? module.target_cluster_registration[0].connection_id : null
  sensitive   = true
}

output "recovery_status" {
  description = "Status message about recovery configuration"
  value = local.is_full_recovery ? (
