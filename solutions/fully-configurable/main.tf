...
module "protect_cluster" {
  ...
  # --- Recovery Settings ---
  enable_recovery                  = var.deployment_mode == "full_backup_recovery" ? true : false
...
locals {
  # Determine which protection group to use for backup/recovery
  # Applicable in full_backup_recovery mode
