...
variable "datacenter" {
  type        = string
  description = "The classic infrastructure datacenter where the cluster is created."
  default     = "dal10"
}
...
variable "cluster_name_id" {
  type        = string
  description = <<EOT
Name or ID of the existing Classic Kubernetes cluster to protect.
If left empty (null, which is the default), this example will automatically create new VLANs
and provision a Classic Kubernetes cluster for you.
