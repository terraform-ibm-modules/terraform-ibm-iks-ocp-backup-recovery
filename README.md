...
| <a name="input_brs_endpoint_type"></a> [brs\_endpoint\_type](#input\_brs\_endpoint\_type) | The endpoint type to use when connecting to the Backup and Recovery service. Allowed values are 'public', 'private', or 'vpe'. Use 'vpe' when a Virtual Private Endpoint Gateway is created via create\_brs\_vpe=true; the module will resolve the BRS endpoint from the VPEG reserved IP DNS name automatically. | `string` | `"private"` | no |
