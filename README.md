...
| <a name="input_create_brs_vpe"></a> [create\_brs\_vpe](#input\_create\_brs\_vpe) | Set to true to create a Virtual Private Endpoint Gateway (VPEG) that routes traffic from the cluster VPC to the BRS instance over the IBM private backbone. When true, vpc\_id and vpc\_subnets must also be provided. For cross-account setups (BRS in a different IBM Cloud account), also provide brs\_source\_account\_id to create the required S2S IAM authorization policy. | `bool` | `false` | no |
