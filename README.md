...
| <a name="input_vpc_id"></a> [vpc\_id](#input\_vpc\_id) | ID of the VPC where the BRS Virtual Private Endpoint Gateway will be created. Required when create\_brs\_vpe is true. | `string` | `null` | no |
| <a name="input_vpc_subnets"></a> [vpc\_subnets](#input\_vpc\_subnets) | List of subnets (one per zone) in which to bind reserved IPs for the BRS VPE Gateway. Each entry must have 'name', 'id', and 'zone'. Required when create\_brs\_vpe is true. Use the subnet\_zone\_list output from terraform-ibm-landing-zone-vpc or equivalent. | <pre>list(object({<br/>    name = string<br/>    id   = string<br/>    zone = string<br/>  }))</pre> | `[]` | no |
