...
| <a name="input_create_dsc_worker_pool"></a> [create\_dsc\_worker\_pool](#input\_create\_dsc\_worker\_pool) | Set to `true` to create a dedicated worker pool for the Data Source Connector in VPC clusters. If set to `false`, the connector will be deployed on existing worker nodes. NOTE: This feature is currently disabled due to Terraform limitations with dynamic zone discovery. | `bool` | `false` | no |
