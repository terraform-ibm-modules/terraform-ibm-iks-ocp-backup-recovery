...
| <a name="input_external_kubeconfig_path"></a> [external\_kubeconfig\_path](#input\_external\_kubeconfig\_path) | Optional external kubeconfig file path. When provided, the cleanup script will use this kubeconfig instead of the one generated by the module. This is useful when the module is called from examples or other contexts where the kubeconfig is managed externally. | `string` | `null` | no |
